...
const CartPage = ({ setIsOpened }: { setIsOpened: (value: boolean) => void }) => {
  ...
  return (
    <div className="flex flex-col h-full bg-white">
      ...
      <div className="flex-grow overflow-auto p-6 scrollbar-thin scrollbar-thumb-green-400 scrollbar-track-white">
        {cartData.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full text-center py-10">
            <ShoppingBag className="text-green-400 mb-4 opacity-30" size={64} />
          ...
          cartData.map((data: any, index: number) => (
            ...
            >
              <div className="flex-shrink-0 w-24 h-24 mr-4">
                <div className="w-full h-full overflow-hidden rounded-md aspect-square border border-zinc-100 group-hover:border-green-400 transition-colors duration-300">
                  <Image
                    width={96}
                    height={96}
                    alt={data.name}
              ...
              <div className="flex-grow">
                <div className="flex justify-between items-start mb-2">
                  <h3 className="text-base-semibold text-zinc-800 group-hover:text-green-500 transition-colors duration-300">
                    {data.name}
                  </h3>
                  <button
                    onClick={() => removeProduct(index)}
                    className="text-zinc-400 hover:text-green-500 transition-colors duration-300 p-1 rounded-full hover:bg-green-50"
                  >
                    <X size={18} />
                  </button>
                </div>
                <div className="flex justify-between items-center">
                  <div className="flex items-center border border-zinc-200 rounded-md overflow-hidden bg-white">
                    <Button
                      onClick={() => minus(index)}
                      variant="ghost"
                      className="p-1 h-8 w-8 text-green-500 hover:text-green-600 hover:bg-green-50"
                    >
                      <Minus size={14} />
                    </Button>
                    <input
                  ...
                  <div className="text-right">
                    ...
                    <p className="text-base-semibold text-green-500">
                      {Store.currency_sign}
                      {data.price}
                    </p>
                  </div>
      ...
      <div className="border-t border-zinc-100 p-6 bg-zinc-50/80 backdrop-blur-sm">
        <div className="flex justify-between items-center mb-6">
          ...
          <span className="text-heading3-bold text-green-500">
            {Store.currency_sign}
            {totalPrice}
          </span>
        </div>
        <div className="space-y-3">
          <Button
            onClick={hideCart}
            variant="outline"
            className="w-full border-green-500 text-green-500 hover:bg-green-50 hover:text-green-600 transition-all duration-300"
          >
          ...
          <Link href="/order" className="block w-full">
            <Button
              ...
              className="w-full bg-green-500 hover:bg-green-400 hover:shadow-md hover:shadow-green-400/20 transition-all duration-300 border-0"
            >
